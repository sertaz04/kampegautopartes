...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<?php
				...
				WHERE a.empresa_id = 1 and (estadoSII is null   or estadoSII='NOK'   or estadoSII='')  and  b.IdTipoDoc in ('33')	and 	s.fecha_factura>='2018-01-01'		
...
$data = file_get_contents($aUrl."?rutConsultante=".urlencode($rutConsultante)."&rutEmpresa=".urlencode($rutEmpresa)."&rutReceptor=".urlencode($rutReceptor)."&tipoDte=".urlencode($tipoDte)."&folioDte=".urlencode($folioDte)."&fechaEmisionDte=".urlencode($fechaEmisionDte)."&montoDte=".urlencode($montoDte)."&empresa=".urlencode(1));
...
if ($tipoDte=='61' || $tipoDte=='33'  || $tipoDte=='56'){
//if ($tipoDte=='33'){
//produccion
...
$data = file_get_contents($aUrl."?empresa=".urlencode(1)."&tipoDocumento=".urlencode($tipoDte)."&numeroDocumento=".urlencode($folioDte)."&enviarSII=".urlencode($EnviarSII)."&customer=".urlencode($customer_id));
echo $aUrl."?empresa=".urlencode(1)."&tipoDocumento=".urlencode($tipoDte)."&numeroDocumento=".urlencode($folioDte)."&enviarSII=".urlencode($EnviarSII)."&customer=".urlencode($customer_id);
// decode data
$json = new Services_JSON;
$obj = $json->decode($data);
}
