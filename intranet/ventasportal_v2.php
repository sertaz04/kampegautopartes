...
function innerSII(tipoDTE,folioDTE)
{
	...
	if($("#formaPago").val()!=''){
		if($("#formaPago").val()==1 || $("#formaPago").val()==6 || $("#formaPago").val()==7){
			...
			if(parseInt($("#saldoMaximo").val()) > parseInt($("#importe").val())){
...
<Button type="submit" class="btn btn-info" style="width: 123px; height:35px; margin-top:-5px;" /><i class="icon-save icon-large <?php if($tipo==24 || ($tipo==26 && $idReferencia!='')){ echo "disabled"; } ?>"></i> agregar</button>
	...
	<br /><br />
		<table class="table table-bordered" id="resultTableNC" data-responsive="table" style="text-align: left;">
            ...
            <tbody>
                    ...
                    <?php
                    include('../connect.php');
                    $result = mysql_query("SELECT v.*, c.customer_name, c.rut, es.estado, td.descripcion, cs.folio_DTE  FROM sales v
                    				...
                    				AND v.`delete`='0' AND (cs.folio_DTE > 0 OR v.invoice_number>0) ORDER BY v.transaction_id DESC");
                        while($row = mysql_fetch_array($result, MYSQL_ASSOC))
                        {
                    ?>
                    <tr class="record">
...
$sql = "SELECT sum((so.cost*so.qty)-(so.cost*so.qty*so.descuento/100)) as deuda FROM sales s
	...
	<tbody>
			...
			<tr>
				...
				$resultas = mysql_query("select sum(t.total) as totalItem  from (
					...
					if($tipoTraslado==5){
						$resultas = mysql_query("select sum(t.total) as totalItem  from (
										select (p.lastcost*so.qty)-(p.lastcost*so.qty*so.descuento/100) as total FROM sales_order so LEFT JOIN products p ON p.product_id = so.product_id WHERE so.invoice= '$sdsd' AND so.empresa_id = $sesionEmpresaID AND so.`delete`='0'
										) t");
					}else{
						$resultas = mysql_query("select sum(t.total) as totalItem  from (
            ...
            <tr>
				...
				<td colspan="2" align="right"><strong style="font-size: 12px; color: #222222;">
                <input type="text" name="importe" id="importe" value="<? echo $sumaItem /*formatMoney($sumaItem, true)*/;?>" readonly>
				</strong></td>
			</tr>
