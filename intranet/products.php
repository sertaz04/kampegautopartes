...
<a  href="index.php"><button class="btn btn-default btn-large" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Volver</button></a>
			<?php 
			include('../connect.php');
				$result = mysql_query("SELECT * FROM products WHERE empresa_id = $sesionEmpresaID AND `delete`='0'");
			...
			include('../connect.php');
					...
					sum(pi.qty)-sum(so.qty) as resto
					from purchases_item pi
					left join sales_order so
					ON so.product = pi.product_id
					WHERE pi.empresa_id = $sesionEmpresaID
					  AND pi.`delete`='0'
...
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	<thead>
		<tr>
			<th width="12%"> Group/Family/Subfamily </th>
			<th width="8%"> C&oacute;digo </th>
			<th width="13%"> Nombre </th>
			<th width="7%"> Costo promedio </th>
			<th width="9%"> &Uacute;ltimo costo </th>
			<th width="10%"> Precio venta </th>
			<th width="9%"> Margen Venta </th>
			<th width="9%"> Precio especial </th>
			<th width="9%"> Margen venta especial </th>
			<th width="6%"> Origen</th>
	...
	<tbody>
			...
			function formatMoney($number, $fractional=false) {
				...
				include('../connect.php');
				$result = mysql_query("SELECT * FROM products p
										LEFT JOIN `group` g  
		   ...
		   $pricesale=$row['pricesale'];
			echo formatMoney($pricesale, true);
		   ?>
           </td>
			<td><?php echo $row['marginsale']; ?></td>
			<td><?php echo $row['pricespecial']; ?></td>
			<td><?php echo $row['marginspecial']; ?></td>
			<td><?php echo $row['originproduct']; ?>	</td>			
           <td><a rel="facebox" title="Click para editar el producto" href="editproduct.php?id=<?php echo $row['product_id']; ?>"><button class="btn btn-warning"><i class="icon-edit"></i> Editar</button> </a>
...
//Built a url to send
var info = 'id=' + del_id;
 if(confirm("! Est&aacute; seguro que desea borrar este producto? Este proceso no es reversible!"))
		  {
