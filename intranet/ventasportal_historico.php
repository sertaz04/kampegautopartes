...
$aUrl = "http://200.73.116.79:8081/api/dte/GetSendStatus/";
...
function innerSII(tipoDTE,folioDTE)
{
	...
	if($("#formaPago").val()!=''){
		...
		alert('FORMA DE PAGO NO DEFINIDA, FAVOR INGRESAR FORMA DE PAGO VÁLIDA');
	}
}
...
function validateForm() {
    ...
    if($("#itemsProducto").val()>20 && <? echo $tipo;?>==14){
        alert("Ha ingresado el límite máximo de elementos para una guía!");
        return false;
    }
}
			...
			$("#btnAddCheque").click(function(){
				...
				$("#fechaPago").val('<?php echo date("Y-m-d");?>');
				 ...
				 <?php
	                ...
	                $result=mysql_query("SELECT * FROM (SELECT * FROM ventas_impago vi INNER JOIN sales s ON s.transaction_id = vi.transaction_id
					UNION ALL
					SELECT * FROM ventas_impago vi INNER JOIN sales_history s ON s.transaction_id = vi.transaction_id) AS F 
					WHERE transaction_id = ".$_GET['iv']);
		...
		function descuento(qty,costo,descuento, venta){
			...
			if(eval($(descuento).val())>eval($("#maximoDesc").val())){
				alert("El valor ingresado como descuento es mayor al permitido, se actualizará");
...
<input type="text" id="busquedaCodigo" placeholder="Código" <?php //if($tipo==24 || $tipo==29){ echo "readonly"; } ?>/>
<input type="text" id="busquedaDescripcion" placeholder="Descripción" size="70" <?php //if($tipo==24 || $tipo==29){ echo "readonly"; } ?>/>
...
<input type="text" name="qty" value="" placeholder="Cantidad" autocomplete="off" style="width: 78px; height:30px; padding-top: 6px; padding-bottom: 6px; margin-right: 4px;" <?php //if($tipo==24 || $tipo==29){ echo "readonly"; } ?>/>
...
{
if($tipo==24){
	echo 'Tipo de documento: Nota de Crédito Electrónica<br>';
	echo 'Transaccion Referencia ID : TR-'.$idReferencia.'<br>';
}else if($tipo==29){
	echo 'Tipo de documento: Nota de Débito Electrónica<br>';
	echo 'Transaccion Referencia ID : TR-'.$idReferencia.'<br>';
}else if($tipo==14){
	echo 'Tipo de documento: Guía de Despacho Electrónica<br>';
	echo 'Transaccion ID : TR-'.$idReferencia.'<br>';
}else if($tipo==26 && $idReferencia!=''){
	echo 'Tipo de documento: Factura Electrónica<br>';
	echo 'Transaccion Referencia ID : TR-'.$idReferencia.'<br>';
}else{
...
<input type="hidden" id="saldoMaximo" name="saldoMaximo" value="<? echo $saldoMaximo;?>" />
        ...
        <?php } //fin forma pago?>
            ...
            <?php
                ...
                $result=mysql_query("select * from (SELECT * FROM ventas_impago vi INNER JOIN sales s ON s.transaction_id = vi.transaction_id WHERE s.transaction_id = $id
                														union all
									SELECT * FROM ventas_impago vi INNER JOIN sales_history s ON s.transaction_id = vi.transaction_id WHERE s.transaction_id = $id
                					) as x ");
		...
		//if ($rowEnvio<=0 && $s==0){
            ...
            <?php }else{?>
            	<div class="alert alert-info">
				  <strong>CLIENTE PERMITE DESCUENTO!</strong> Este cliente permite un descuento máximo de: <?php echo $maximoDesc;?>.
				</div>
			<?php }?>
            <?php if($saldoMaximo>0){?>
	            <div class="alert alert-success">
				  <strong>CLIENTE CON CUENTA CORRIENTE!</strong> Este cliente permite pago con crédito como máximo de: <?php echo $saldoMaximo;?>. Deuda de: <?php echo $deudaTotal;?>
				</div>
            <?php }?>
            <?php } //tipo documento?>
...
<? 	}else{
	...
	<button name="myBtnFactElect" id="myBtnFactElect" style="height:35px; float:right;" class="btn btn-success btn-large <?php if($bloqueado!=""){ echo "disabled";}?>" onClick="">Factura Electrónica</button>
