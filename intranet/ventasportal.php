...
//CONSULTA ESTADO ENVIO
if (isset($_POST['valido']) and $_POST['valido']=="ConsultaSII"){

require_once("json.php");

$tipoDocto = $_GET['tipoDTE'];
$idEmpresa = $sesionEmpresaID;
$folio = $_GET['folioDTE'];
$usuario=$_SESSION['KT_Username'];
$fechaActual = date("d-m-Y");

$rutEmisor = "78753420-1";//strtoupper($_POST['rutEmisor']);
$trackID = $_POST['trackID'];
echo "<h2>Respuesta Consulta Estado</h2>\n";

$aUrl = "http://190.14.56.35:8081/api/dte/GetSendStatus/";
if (isset($_POST['rutEmisor'])){
  $rutEmisor = urlencode($_POST['rutEmisor']);
}
$data = file_get_contents($aUrl."?rutEmpresa=".strtoupper($rutEmisor)."&trackId=".$folio);
// decode data
...
//echo $aUrl."?empresa=".urlencode($idEmpresa)."&tipoDocumento=".urlencode($tipoDocto)."&numeroDocumento=".urlencode($folio)."&enviarSII=".urlencode($EnviarSII)."&customer=".urlencode($customer);
// decode data
$json = new Services_JSON;
$obj = $json->decode($data);
}
...
//CONSULTO SI DTE YA TIENE UN FOLIO ASIGNADO PARA NO GENERAR OTRO - ESTO CUANDO SE PRODUCCE FALLO EN ENVIO AL SII
$result = mysql_query("
		SELECT folio_DTE
		FROM claveSII 
		WHERE id_invoice='$id' and tipo_documento= '$tipo' AND empresa_id = '$sesionEmpresaID'");
$rowSIIBoton1 = mysql_num_rows($result); //mysql_fetch_array($result, MYSQL_ASSOC);
$rowSII1=mysql_fetch_array($result, MYSQL_ASSOC);
...
//CONSULTO SI ESTA ENVIADO DTE AL SII
$resultCompruebaEnvio = mysql_query("
		SELECT * FROM DTE_envioSII 
		WHERE TipoDocto= '$tipo' AND Empresa = '$sesionEmpresaID' AND NroDTE='$folio_nvo'");
		$rowEnvios = 0; 
$rowEnvio = mysql_fetch_array($resultCompruebaEnvio, MYSQL_ASSOC);

//CONSULTO SI DTE YA TIENE UN FOLIO ASIGNADO PARA NO GENERAR OTRO - ESTO CUANDO SE PRODUCCE FALLO EN ENVIO AL SII
$result = mysql_query("
		SELECT folio_DTE
		FROM claveSII 
		WHERE id_invoice='$idRef' and tipo_documento= '$tipo' AND empresa_id = $sesionEmpresaID");
 $rowSIIBoton1 = mysql_num_rows($result); //mysql_fetch_array($result, MYSQL_ASSOC);
$rowSII1=mysql_fetch_array($result, MYSQL_ASSOC);

$s=0;
if ($rowSIIBoton1>0){ 
			$folio_nvo=$rowSII1['folio_DTE'];
			$s=1;
}
...
$result = mysql_query("INSERT 
				...
				values($idRef, $folio_nvo,'".date("Y-m-d")."',$tipo, 1, $sesionEmpresaID, '$username')");
$folio = $folio_nvo;
}else{
...
function consultaSII(rutEmisor,dv, trackID)
{
//alert("hugo");
document.getElementById('Div1').innerHTML = "<input name='valido' value='ConsultaSII' type='hidden'><input name='rutEmisor' value='' type='hidden'><input name='trackID' value='' type='hidden'>";

document.forms["myForm"]["valido"].value="ConsultaSII";
document.forms["myForm"]["folioDTE"].value=folioDTE;
document.forms["myForm"]["tipoDTE"].value=tipoDTE;
						
document.forms.form1.rutEmisor.value=rutEmisor;
document.forms.form1.trackID.value=trackID;
alert("SUPER");
document.forms.myForm.submit();
}

function innerSII(tipoDTE,folioDTE)
{

					<?php include('../connect.php');
			        $resultSuspend=mysql_query("SELECT * FROM DTE_informacionEmpresas WHERE CodEmpresa = $sesionEmpresaID");
			        //$rowCountSuspend = mysql_num_rows($resultSuspend);   
					$rowSuspend=mysql_fetch_array($resultSuspend, MYSQL_ASSOC);
					//$pagoOn=$rowSuspend['Pago'];	
					?>
				$pagoOn = <?php echo $rowSuspend['Pago']; ?>;
	...
	//VALIDA FORMA PAGO
	if($("#formaPago").val()!=''){
		if($("#formaPago").val()==1 || $("#formaPago").val()==6 || $("#formaPago").val()==7){
			alert ("ENVIO DTE AL SII EN PROCESO");
			//document.forms["myForm"]["qty"].value
			//alert ("paso 2"); EnvioSIIBorrador
			document.forms["myForm"]["valido"].value="EnvioSII";
			document.forms["myForm"]["folioDTE"].value=folioDTE;
			document.forms["myForm"]["tipoDTE"].value=tipoDTE;
			//alert ("paso 3");
			document.forms.myForm.submit();
			...
			if((parseInt($("#saldoMaximo").val()) > parseInt($("#importe").val())) || <? echo $tipo;?>==24 || <? echo $tipo;?>==29){
		
				//VALIDA FORMA PAGO CHEQUE CON CHEQUE INGRESADO
				if($("#formaPago").val()==3 || $("#formaPago").val()==4 || $("#formaPago").val()==5){ 
					<?php include('../connect.php');
			        $result=mysql_query("SELECT * FROM ventas_impago vi INNER JOIN sales s ON s.transaction_id = vi.transaction_id WHERE s.transaction_id = $id");
			        $rowCountCheque = mysql_num_rows($result);       
					?>
					$rowCountCheque = <? if($rowCountCheque!=''){ echo $rowCountCheque; }else{ echo 0;}?>;
					if($rowCountCheque>0){
				...
				alert('CUPO INSUFICIENTE PARA REALIZAR COMPRA!');
			}
		}
	}else{
...
function validateForm() {
    ...
    if($("#itemsProducto").val()>20 && <? echo $tipo;?>==14){
        alert("Ha ingresado el l�mite m�ximo de elementos para una gu�a!");
        return false;
    }
}
		...
		$(document).ready(function(){
				...
				$("#fechaPago").val('<?php echo date("Y-m-d");?>');
				 ...
				 <?php
	                ...
	                $resultas = mysql_query("select sum(t.total) as totalItem  from (
	                		select (((cost*qty)-(ROUND(cost*qty*descuento/100)))*1.19) as total from sales_order WHERE invoice= '".$_GET['iv']."' AND empresa_id = $sesionEmpresaID AND `delete`='0'
							UNION ALL
							select (((cost*qty)-(ROUND(cost*qty*descuento/100)))*1.19) as total from sales_order_history WHERE invoice= '".$_GET['iv']."' AND empresa_id = $sesionEmpresaID AND `delete`='0'
	                		) t");
					...
					WHERE vi.transaction_id = ".$_GET['iv']);
					...
					//history
					$result=mysql_query("SELECT * FROM ventas_impago vi INNER JOIN sales_history s ON s.transaction_id = vi.transaction_id WHERE vi.transaction_id = ".$_GET['iv']);
	                $totalVentaCh = $sumaItem;
	                while($row = mysql_fetch_array($result, MYSQL_ASSOC)){
                    	$totalVentaCh -= $row['monto'];
                    }
			...
			$("#myBtnGuardar").click(function(){
				...
				<?php }elseif($tipo==29 && $_GET['edita']==''){?>
					form.setAttribute("action", "saveventaND.php");
					//form.setAttribute("action", "saveitemtodo.php");
				<?php }elseif($tipo==26 && $idReferencia!=''){?>
					form.setAttribute("action", "saveventaGuia.php");
				<?php }else{?>
					form.setAttribute("action", "saveitemtodo.php");
				<?php }?>
				...
				form.appendChild(document.getElementById("orden_compra"));
		...
		function descuento(qty,costo,descuento, venta){
			...
			$(venta).val( $(qty).val()*$(costo).val()-(Math.round($(qty).val()*$(costo).val()*($(descuento).val()/100))) );
			calcular_total();
		}
...
$(document).ready(function(){
    ...
    //comprobamos si se pulsa una tecla
    $("#busquedaCodigoBarra").blur(function(e){
									
          //obtenemos el texto introducido en el campo de b�squeda
		  $consultaCodigoBarra = $("#busquedaCodigoBarra").val();
		  
		  if($consultaCodigoBarra!=null && $consultaCodigoBarra!=''){
			  
			  $grupoCodigoBarra  = $consultaCodigoBarra.slice(0, 1);
			  $familiaCodigoBarra  = $consultaCodigoBarra.slice(1, 3);
			  $subfamiliaCodigoBarra  = $consultaCodigoBarra.slice(3, 5);
			  $codigoCodigoBarra  = $consultaCodigoBarra.slice(5);
			  
			  
			  $("#busquedaGrupo").val($grupoCodigoBarra);
			  $("#busquedaFamily").val($familiaCodigoBarra);
			  $("#busquedaSubfamily").val($subfamiliaCodigoBarra);
			  $("#busquedaCodigo").val($codigoCodigoBarra);
			  ...
			  $.ajax({
					type: "POST",
					url: "buscar.php",
					data: {grupo:$grupoCodigoBarra,familia:$familiaCodigoBarra,subfamily:$subfamiliaCodigoBarra,codigo:$codigoCodigoBarra, descripcion:''},
					beforeSend: function(){
						  //imagen de carga
						$("#resultado").html("<p align='center'><img src='../img/loader.gif' /></p>");
					},
					...
					success: function(data){       
						$("#resultado").empty();                                             
						  ...
						  //enviar a guardar el formulario para agregar a la venta
						  $("#qtyForm").val('1');
...
<?php if(($tipo!=24 && $tipo!=29) || ($tipo==24 && $idReferencia!='') || ($tipo==29 && $idReferencia!='')){ //distinto de nota credito o debito?>

<form action="saveventasitem.php" method="post" onSubmit="return validateForm()" name="myForm">
  <div id='Div1'></div>

C&oacute;digo de Barra : 
<input type="text" id="busquedaCodigoBarra" placeholder="C�digo de Barra" size="70" <?php //if($tipo==24 || $tipo==29){ echo "readonly"; } ?>/>
 <br /><br />
<input type="hidden" name="invoice" value="<?php echo $id; ?>" />
<input type="hidden" name="invoiceReference" value="<?php echo $idReferencia; ?>" />
<input type="hidden" name="tipo" value="<?php echo $tipo; ?>" />
<input type="text" id="busquedaGrupo" placeholder="Grupo" maxlength="2" size="2" <?php //if($tipo==24 || $tipo==29){ echo "readonly"; } ?>/>
<input type="text" id="busquedaFamily" placeholder="Familia" maxlength="2" size="2" <?php //if($tipo==24 || $tipo==29){ echo "readonly"; } ?>/>
<input type="text" id="busquedaSubfamily" placeholder="Subfamilia" maxlength="2" size="2" <?php //if($tipo==24 || $tipo==29){ echo "readonly"; } ?>/>
<input type="text" id="busquedaCodigo" placeholder="C�digo" <?php //if($tipo==24 || $tipo==29){ echo "readonly"; } ?>/>
<input type="text" id="busquedaDescripcion" placeholder="Descripci�n" size="70" <?php //if($tipo==24 || $tipo==29){ echo "readonly"; } ?>/>
...
<input type="text" name="qty" id="qtyForm" value="" placeholder="Cantidad" autocomplete="off" style="width: 78px; height:30px; padding-top: 6px; padding-bottom: 6px; margin-right: 4px;" <?php //if($tipo==24 || $tipo==29){ echo "readonly"; } ?>/>
<input type="hidden" id="cliente_id" name="cliente_id" value="<? echo $_GET['sp'];?>" />
<Button type="submit" class="btn btn-info <?php if(/*$tipo==24 ||*/ ($tipo==26 && $idReferencia!='') || ($bloqueado!="" || ($_GET['edita']=='datos' && $numeroVenta!=''))){ echo "disabled"; } ?>" style="width: 123px; height:35px; margin-top:-5px;" /><i class="icon-save icon-large"></i> agregar</button>
	...
	<br /><br />
		<table class="table table-bordered" id="resultTableNC" data-responsive="table" style="text-align: left;">
            ...
            <tbody>
					...
					if($tipo==29){
                    $result = mysql_query("SELECT * FROM (SELECT v.*, c.customer_name, c.rut, es.estado, td.descripcion, cs.folio_DTE  FROM sales v
                    				...
                    				WHERE v.empresa_id = $sesionEmpresaID AND v.tipo_documento_id in (12, 26, 24)
                    				AND v.`delete`='0' AND (cs.folio_DTE > 0 OR v.invoice_number>0) 
									union all
									SELECT v.*, c.customer_name, c.rut, es.estado, td.descripcion, cs.folio_DTE  FROM sales_history v
                    				left join customer c on c.customer_id = v.customer_id
                    				left join claveSII cs on cs.id_invoice = v.transaction_id
                    				left join estadoSII es on es.id = cs.estado
                    				left join tipo_documento td on v.tipo_documento_id=td.tipo_documento_id
                    				WHERE v.empresa_id = $sesionEmpresaID AND v.tipo_documento_id in (12, 26, 24)
                    				AND v.`delete`='0' AND (cs.folio_DTE > 0 OR v.invoice_number>0)) AS VENTAS
					 ...
					 $result = mysql_query("SELECT * FROM (SELECT v.*, c.customer_name, c.rut, es.estado, td.descripcion, cs.folio_DTE  FROM sales v
                    				...
                    				left join estadoSII es on es.id = cs.estado
                    				left join tipo_documento td on v.tipo_documento_id=td.tipo_documento_id
                    				WHERE v.empresa_id = $sesionEmpresaID AND v.tipo_documento_id in (12, 26)
                    				AND v.`delete`='0' AND (cs.folio_DTE > 0 OR v.invoice_number>0) 
									UNION ALL
									SELECT v.*, c.customer_name, c.rut, es.estado, td.descripcion, cs.folio_DTE  FROM sales_history v
                    				left join customer c on c.customer_id = v.customer_id
                    				...
                    				WHERE v.empresa_id = $sesionEmpresaID AND v.tipo_documento_id in (12, 26)
                    				AND v.`delete`='0' AND (cs.folio_DTE > 0 OR v.invoice_number>0) ) AS VENTAS
									ORDER BY descripcion, folio_DTE DESC");
					}
                    ...
                    <td><?php if($row['tipo_documento_id']==12){ echo $row['invoice_number']; }else{ echo $row['folio_DTE']; }?></td>
                    ...
                    <?php if($tipo==24){?>
						...
						<a href="saveventaNC4.php?cliente=<?php echo $row['customer_id']; ?>&ivReferencia=<?php echo $row['transaction_id']; ?>" id="<?php echo $row['transaction_id']; ?>" class="anulabutton" title="Click para editar DTE"><button class="btn btn-primary btn-mini"><i class="icon-trash"></i> Edita Monto </button></a>
	                    ...
	                    <a href="saveventaNC3.php?cliente=<?php echo $row['customer_id']; ?>&ivReferencia=<?php echo $row['transaction_id']; ?>" id="<?php echo $row['transaction_id']; ?>" class="anulabutton" title="Click para anular DTE"><button class="btn btn-danger btn-mini"><i class="icon-trash"></i> Anula DTE </button></a>
                    <?php }else if($tipo==29){
								if($row['tipo_documento_id']==24){
							?>
							<a href="saveventaND5.php?cliente=<?php echo $row['customer_id']; ?>&ivReferencia=<?php echo $row['transaction_id']; ?>" id="<?php echo $row['transaction_id']; ?>" class="anulabutton" title="Click para anular DTE"><button class="btn btn-danger btn-mini"><i class="icon-trash"></i> Anula DTE </button></a>
								<? }else{ ?>
							<a href="saveventaND4.php?cliente=<?php echo $row['customer_id']; ?>&ivReferencia=<?php echo $row['transaction_id']; ?>" id="<?php echo $row['transaction_id']; ?>" class="anulabutton" title="Click para editar DTE"><button class="btn btn-primary btn-mini"><i class="icon-trash"></i> Edita Monto </button></a>
								<? } ?>							
...
$maximoDesc=0;
$saldoMaximo=0;
if($tipo==24 || $tipo==29 || ($tipo==14 && $idReferencia!='') || ($tipo==26 && $idReferencia!='')){
	$resultaz = mysql_query("SELECT * FROM (SELECT v.*, c.rut, c.customer_name, c.descuento, c.porcentaje_maximo_descuento, c.saldo_maximo, c.bloqueado, td.descripcion, cs.folio_DTE FROM sales v left join claveSII cs on cs.id_invoice = v.transaction_id and cs.tipo_documento=$tipo left join customer c on v.customer_id = c.customer_id LEFT JOIN tipo_documento td ON td.tipo_documento_id = v.tipo_documento_id WHERE v.transaction_id= '$idReferencia' and c.customer_id='$customer' AND v.empresa_id = $sesionEmpresaID AND v.`delete`='0'
	UNION ALL
	SELECT v.*, c.rut, c.customer_name, c.descuento, c.porcentaje_maximo_descuento, c.saldo_maximo, c.bloqueado, td.descripcion, cs.folio_DTE FROM sales_history v left join claveSII cs on cs.id_invoice = v.transaction_id and cs.tipo_documento=$tipo left join customer c on v.customer_id = c.customer_id LEFT JOIN tipo_documento td ON td.tipo_documento_id = v.tipo_documento_id WHERE v.transaction_id= '$idReferencia' and c.customer_id='$customer' AND v.empresa_id = $sesionEmpresaID AND v.`delete`='0') AS VENTAS");
}else{
...
if($tipo==24){
	...
	echo 'Tipo de documento: Nota de D�bito Electr�nica<br>';
	echo 'Transaccion Referencia ID : TR-'.$idReferencia.'<br>';
}else if($tipo==14){
	echo 'Tipo de documento: Gu�a de Despacho Electr�nica<br>';
	echo 'Transaccion ID : TR-'.$idReferencia.'<br>';
}else if($tipo==26 && $idReferencia!=''){
	echo 'Tipo de documento: Factura Electr�nica<br>';
	echo 'Transaccion Referencia ID : TR-'.$idReferencia.'<br>';
}else{
...
$tipoTraslado = $rowaz['tipo_traslado_id'];
$clienteId = $rowaz['customer_id'];
$idNota = $rowaz['folio_DTE'];

include('../connect.php');
//select deuda pendiente cliente

$sql = "SELECT sum(deuda) as deuda FROM (SELECT sum((so.cost*so.qty)-(ROUND(so.cost*so.qty*so.descuento/100))) as deuda FROM sales s
...
LEFT JOIN customer c ON c.customer_id = s.customer_id
  ...
  AND s.`delete`=0
  UNION ALL
  SELECT sum((so.cost*so.qty)-(ROUND(so.cost*so.qty*so.descuento/100))) as deuda FROM sales_history s
...
$resultDeuda = mysql_query($sql);
$deudaTotal = 0;
while($rowDeuda = mysql_fetch_array($resultDeuda, MYSQL_ASSOC))
{
	$deudaTotal = $rowDeuda['deuda'];
}
}
$id=$_GET['iv'];
...
<input type="hidden" id="saldoMaximo" name="saldoMaximo" value="<? echo $saldoMaximo;?>" />
        ...
        <?php } //fin forma pago?>
			...
			<?
                      $result=mysql_query("SELECT * FROM ventas_impago vi INNER JOIN sales_history s ON s.transaction_id = vi.transaction_id WHERE s.transaction_id = $id");
...
$resultCompruebaEnvio = mysql_query("
		SELECT * FROM DTE_envioSII 
		WHERE TipoDocto= '$tipo' AND Empresa = '$sesionEmpresaID' AND NroDTE='$id'");
		$rowEnvios = mysql_num_rows($resultCompruebaEnvio);
$rowEnvio = mysql_fetch_array($resultCompruebaEnvio, MYSQL_ASSOC);
...
if ($tipo=="24" || $tipo=="29"){
	$result = mysql_query("
		SELECT folio_DTE
		FROM claveSII 
		WHERE id_invoice='$idReferencia' and tipo_documento= '$tipo' AND empresa_id = $sesionEmpresaID");
	}else{
		$result = mysql_query("
		SELECT folio_DTE
		FROM claveSII 
		...
		$rowSIIBoton = mysql_num_rows($result); 
		//mysql_fetch_array($result, MYSQL_ASSOC);
		$rowSII=mysql_fetch_array($result, MYSQL_ASSOC);
		if ($rowSIIBoton>0 ){ //and $rowEnvios>0
		//if ($rowEnvio<=0 && $s==0){
			$idRef=$rowSII['folio_DTE'];
			$sw=1;
		}else{
					if ($tipo=="24" || $tipo=="29"){
						$idRef = $idReferencia;
				  ...
				  $rutaXMLSII="C:/inetpub/wwwroot/App_Data/Resources/XML/es-CL/".$tipoDTE."_".$idRef."_".$idEmp.".EnvioDTESigned.xml";
					...
					$urlexistsBorrador = url_exists($rutaBorrador);
					$urlexistsCedible = url_exists($rutaPDFCedible);
					$urlexistsNCedible = url_exists($rutaPDFNCedible);
					if ($sw == 0){
					  if ($urlexistsBorrador) {
					  //		echo"<a href='#' onClick='innerDet($tipoDTE,$folio);'>DET.FACT.</a> - ";
							echo"<a href='".$rutaBorrador."' target='_blank'>PDF BORRADOR</a> ";
					   }
				   }
				   if ($sw == 1){
					  if ($urlexistsCedible) { ?>
					  
					  		<button style="height:35px; float:right;" class="btn btn-success btn-large" onClick='javascript:innerSII(<?php //echo $tipoDTE; ?>,<?php //echo $idRef; ?>);'>CEDER CREDITOS</button>
					 <?php  //		echo"<a href='#' onClick='innerDet($tipoDTE,$folio);'>DET.FACT.</a> - ";
							//echo"<a href='".$rutaPDF."' target='_blank'>PDFs</a> - ";
							echo"<a href='".$rutaPDFCedible."' target='_blank'>PDF CEDIBLE</a> - ";
					   }
					   if ($urlexistsNCedible) {
					  //		echo"<a href='#' onClick='innerDet($tipoDTE,$folio);'>DET.FACT.</a> - ";
							echo"<a href='".$rutaPDFNCedible."' target='_blank'>PDF NO CEDIBLE</a> - ";
					   }
					   
					   if ($rutaXMLA) {
					  //		echo"<a href='#' onClick='innerDet($tipoDTE,$folio);'>DET.FACT.</a> - ";
							echo"<a href='".$rutaXMLA."' target='_blank'>XML CLIENTE</a> - ";
					   }
					  // if ($rutaXMLS) {
					  //		echo"<a href='#' onClick='innerDet($tipoDTE,$folio);'>DET.FACT.</a> - ";
						//	echo"<a href='".$rutaXMLS."' target='_blank'>XML SII</a> ";
					 //  }
            ...
            <?php if($saldoMaximo>0){?>
	            <div class="alert alert-success">
				  <strong>CLIENTE CON CUENTA CORRIENTE!</strong> Este cliente permite pago con cr�dito como m�ximo de: <?php echo $saldoMaximo;?>. Deuda de: <?php echo $deudaTotal;?>
				</div>
            <?php }?>
            <?php } //tipo documento?>
...
<?php if(($tipo!=24 && $tipo!=29) || ($tipo==24 && $idReferencia!='') || ($tipo==29 && $idReferencia!='')){ ?>
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	<thead>
		<tr>
			<th width="35%"> Nombre </th>
			<th width="10%"> Cantidad </th>
			<th width="15%"> <?php if($tipoTraslado==5){
										echo "P. Costo";
									}else{
										echo "P. Unitario";
									}
	...
	<tbody>
			...
			<?php
				...
				if($tipo==24 || $tipo==29 || ($tipo==26 && $idReferencia!='')){
					$result = mysql_query("select * from (SELECT * FROM sales_order WHERE invoice= '$idReferencia' AND empresa_id = $sesionEmpresaID AND `delete`='0'
					union all 
					SELECT * FROM sales_order_history WHERE invoice= '$idReferencia' AND empresa_id = $sesionEmpresaID AND `delete`='0') as f");
				}else{
					$result = mysql_query("select * from (SELECT * FROM sales_order WHERE invoice= '$id' AND empresa_id = $sesionEmpresaID AND `delete`='0'
					UNION ALL
					SELECT * FROM sales_order_history WHERE invoice= '$id' AND empresa_id = $sesionEmpresaID AND `delete`='0') AS F");
				}
				while($row = mysql_fetch_array($result, MYSQL_ASSOC))
				{
					$itemsmProducto +=1;
			...
			<?php if($tipoTraslado!=5){ ?>
				<td>
					<input type="text" name="pventa-<?php echo $row['id']; ?>" id="pventa-<?php echo $row['id']; ?>" value="<?php 
					if($row['cost']>0){
						if($tipoTraslado!=5){
							$dfdf=round($row['cost']*1.19);
						}
					}else{
						$dfdf=round($rowss['pricesale']*1.19);
					}
					echo $dfdf;
					?>" >
				</td>
            ...
            <?php
				...
				$total=round($costoItem*$row['qty']-(round($costoItem*$row['qty']*$row['descuento']/100)));
			}
            ?>
				<input type="text" name="ventas-<?php echo $row['id']; ?>" id="ventas-<?php echo $row['id']; ?>" value="<? echo $total; ?>"
				onChange="descuento('#qty-<?php echo $row['id'];?>', '#cost-<?php echo $row['id'];?>' ,'#descuento-<?php echo $row['id'];?>', '#ventas-<?php echo $row['id']; ?>')" readonly>
			
            </td>
			<td>
			<?php if (!$urlexistsCedible) { ?>
			<center>
			<a href="<?php 
						if($tipo==24 || ($tipo==26 && $idReferencia!='')){ ?>
							deletepventa.php?id=<?php echo $row['id']; ?>&invoice=<?php echo $_GET['ivReferencia']; ?>&qty=<?php echo $row['qty'];?>&code=<?php echo $row['name'];?>&tipo=24&sp=<?php echo $customer; 
					    }else if($tipo==29){ ?>
							deletepventa.php?id=<?php echo $row['id']; ?>&invoice=<?php echo $_GET['ivReferencia']; ?>&qty=<?php echo $row['qty'];?>&code=<?php echo $row['name'];?>&tipo=29&sp=<?php echo $customer; 
						}else{ ?>  
							deletepventa.php?id=<?php echo $row['id']; ?>&invoice=<?php echo $_GET['iv']; ?>&qty=<?php echo $row['qty'];?>&code=<?php echo $row['name'];?>&tipo=<?php echo $tipo;?>&sp=<?php echo $customer;}?>"
			>
			    <button class="btn btn-danger btn-mini <?php if($tipo==26 && $idReferencia!=''){ echo "disabled"; } ?>"><i class="icon-trash"></i> Borrar </button>
			</a>
						<?php
				}
			?></td>
			...
			?>
			<input type="hidden" id="itemsProducto" name="itemsProducto" value="<?php echo $itemsmProducto;?>">
			<tr>
				...
				if($tipo==24 || $tipo==29 || ($tipo==26 && $idReferencia!='')){
				$resultas = mysql_query("select sum(t.total) as totalItem  from (
										select ROUND((cost*qty)-(cost*qty*descuento/100)) as total from sales_order WHERE invoice= '$idReferencia' AND empresa_id = $sesionEmpresaID AND `delete`='0'
										UNION ALL
										select ROUND((cost*qty)-(cost*qty*descuento/100)) as total from sales_order_history WHERE invoice= '$idReferencia' AND empresa_id = $sesionEmpresaID AND `delete`='0'
										) t");
				}else{
					
					if($tipoTraslado==5){
						$resultas = mysql_query("select sum(t.total) as totalItem  from (
										select ROUND((p.lastcost*so.qty)-(p.lastcost*so.qty*so.descuento/100)) as total FROM sales_order so LEFT JOIN products p ON p.product_id = so.product_id WHERE so.invoice= '$sdsd' AND so.empresa_id = $sesionEmpresaID AND so.`delete`='0'
										UNION ALL
										select ROUND((p.lastcost*so.qty)-(p.lastcost*so.qty*so.descuento/100)) as total FROM sales_order_history so LEFT JOIN products p ON p.product_id = so.product_id WHERE so.invoice= '$sdsd' AND so.empresa_id = $sesionEmpresaID AND so.`delete`='0'
										) t");
					}else{
						$resultas = mysql_query("select sum(t.total) as totalItem  from (
								select ROUND((cost*qty)-(cost*qty*descuento/100)) as total from sales_order WHERE invoice= '$sdsd' AND empresa_id = $sesionEmpresaID AND `delete`='0'
								UNION ALL
								select ROUND((cost*qty)-(cost*qty*descuento/100)) as total from sales_order_history WHERE invoice= '$sdsd' AND empresa_id = $sesionEmpresaID AND `delete`='0'
								) t");
					}
				}
				...
				$sumaItem =round($sumaItem);
				?>
                 <input type="text" name="neto" id="neto" value="<? echo $neto /*formatMoney($neto, true)*/;?>" readonly>
				</strong></td>
			</tr>
            <tr>
				...
				?>
                 <input type="text" name="iva" id="iva" value="<? echo $iva /*formatMoney($iva, true)*/;?>" readonly>
				</strong></td>
			</tr>
            <tr>
            ...
            <tr>
				...
				<td colspan="2" align="right"><strong style="font-size: 12px; color: #222222;">
                <input type="text" name="importe" id="importe" value="<? echo $sumaItem+$iva+$adicional /*formatMoney($sumaItem, true)*/;?>" readonly>
				</strong></td>
			</tr>
		
	</tbody>
</table></div><br>
<span>Orden de Compra</span>&nbsp;&nbsp;&nbsp;<input type="text" name="orden_compra" id="orden_compra" placeholder="Orden de Compra" maxlength="18" value="<?php echo $orden_compra;?>"/><br />
...
<textarea name="numeroPalabras" id="numeroPalabras" placeholder="Numero en palabras" style="margin: 0px; width: 803px; height: 55px;"><?php echo $sumaItem>0 ? numtoletras($sumaItem+$iva+$adicional) : ''; ?></textarea>
...
<? 	}else{
	...
	?>
<?php	
if ($rowSIIBoton<=0 or $rowSIIBoton==''  or $sww=1){ //  or $sww=1 
	if($tipo!=13 && $tipo!=15){?>
	...
	<button style="height:35px; float:right;" class="btn btn-success btn-large" onClick='javascript:innerSII(<?php echo $tipoDTE; ?>,<?php echo $idRef; ?>);'>Enviar al SII</button>
	
	<!--	<button style="height:35px; float:right;" class="btn btn-success btn-large" onClick='javascript:consultaSII(<?php //echo $tipoDTE; ?>,<?php //echo $idRef; ?>);'>Consulta Estado SII</button>-->
	 
	<?php }?>
	<button name="myBtnGuardar" id="myBtnGuardar" style="height:35px; float:right;" class="btn btn-success btn-large <?php if($bloqueado!="" || ($_GET['edita']=='datos' && $numeroVenta!='')){ echo "disabled";}?>" onClick="">Guardar</button>
	<?php } ?>
	<?php }?>
<?php } ?>
