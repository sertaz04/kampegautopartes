...
<div style="margin-top: -19px; margin-bottom: 21px;">
			...
			require_once("json.php");
				$result = mysql_query("SELECT * FROM `DTE_caf` WHERE Empresa = 2 and Vigente='SI' ORDER BY TipoDocto DESC");
...
(round(((`so`.`cost` * `so`.`qty`) - (((`so`.`cost` * `so`.`qty`) * `so`.`descuento`) / 100)),0) + round(((((`so`.`cost` * `so`.`qty`) - (((`so`.`cost` * `so`.`qty`) * `so`.`descuento`) / 100)) * 19) / 100),0)) AS `Total`
				...
				WHERE a.empresa_id = 2 and (estadoSII is null   or estadoSII='NOK'   or estadoSII='')  and  b.IdTipoDoc in ('33')	and 	s.fecha_factura>='2018-01-01'		
			...
			/*	echo "SELECT a.fecha, a.folio_DTE, a.estadoSII, b.IdTipoDoc, b.NombreDoc, s.customer_id, c.customer_name FROM `claveSII` as a
				...
				join customer as c on c.customer_id=s.customer_id
				WHERE a.empresa_id = 2 and estadoSII is null ORDER BY b.documento_id DESC";
				*/
...
$data = file_get_contents($aUrl."?rutConsultante=".urlencode($rutConsultante)."&rutEmpresa=".urlencode($rutEmpresa)."&rutReceptor=".urlencode($rutReceptor)."&tipoDte=".urlencode($tipoDte)."&folioDte=".urlencode($folioDte)."&fechaEmisionDte=".urlencode($fechaEmisionDte)."&montoDte=".urlencode($montoDte)."&empresa=".urlencode(2));

//echo $aUrl."?rutConsultante=".urlencode($rutConsultante)."&rutEmpresa=".urlencode($rutEmpresa)."&rutReceptor=".urlencode($rutReceptor)."&tipoDte=".urlencode($tipoDte)."&folioDte=".urlencode($folioDte)."&fechaEmisionDte=".urlencode($fechaEmisionDte)."&montoDte=".urlencode($montoDte)."&empresa=".urlencode(2);
// decode data
...
if ($posDnk !== false) {
	...
	$resultUpdate=mysql_query("UPDATE claveSII as a
        SET estadoSII='$estado' WHERE id_invoice='$id' and a.empresa_id = 2");
}elseif ($posDok !== false) {
    $estado='DOK';
...
$data = file_get_contents($aUrl."?empresa=".urlencode(2)."&tipoDocumento=".urlencode($tipoDte)."&numeroDocumento=".urlencode($folioDte)."&enviarSII=".urlencode($EnviarSII)."&customer=".urlencode($customer_id));
echo $aUrl."?empresa=".urlencode(2)."&tipoDocumento=".urlencode($tipoDte)."&numeroDocumento=".urlencode($folioDte)."&enviarSII=".urlencode($EnviarSII)."&customer=".urlencode($customer_id);
// decode data
$json = new Services_JSON;
$obj = $json->decode($data);
}
