...
$result = mysql_query("UPDATE sales_history SET adicional=$c, observaciones='$b', forma_pago_id=$a WHERE transaction_id=$i");
...
foreach ($arrayIds as $idItem) {
	...
	$resultSELECT=mysql_query("SELECT * FROM sales_order_history WHERE id=$h AND `delete`='0'");
	while($row = mysql_fetch_array($resultSELECT, MYSQL_ASSOC))
	{
		$ab1 = $row['product_id'];
		//$ab2 = $row['qty'];
