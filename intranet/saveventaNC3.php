...
$resultSELECT=mysql_query("SELECT transaction_id FROM sales WHERE invoice_reference = '$c1' AND tipo_documento_id = 24 AND `delete`='0'");
...
/*echo "INSERT INTO sales (invoice_number, invoice_reference, causa_emision_id,correlativo, fecha_factura, fecha_vencimiento, 
						...
						FROM sales WHERE transaction_id = $c1";
*/
$resultINS=mysql_query("INSERT INTO sales_history (invoice_number, invoice_reference, causa_emision_id,correlativo, fecha_factura, fecha_vencimiento, 
											fecha_ingreso, customer_id,tipo_productos, centro_id, empresa_id, sucursal_id, tipo_documento_id, 
											user_create)
						SELECT '', '$c1', '1', '$d', '".date("Y-m-d")."', '".date("Y-m-d")."', '".date("Y-m-d")."', customer_id, tipo_productos, 
								centro_id, empresa_id, sucursal_id, 24, '$username'
						FROM sales_history WHERE transaction_id = $c1");
								
$resultSELECT=mysql_query("SELECT transaction_id FROM sales_history WHERE invoice_reference = '$c1' AND tipo_documento_id = 24 AND `delete`='0'");
while($row = mysql_fetch_array($resultSELECT, MYSQL_ASSOC))
{
	$tid=$row['transaction_id'];
}
...
header("location: ventasportal.php?tipo=24&ivReferencia=$tid&sp=$a&edita=datos");
